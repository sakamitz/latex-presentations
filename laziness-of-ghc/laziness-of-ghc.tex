...
\inlinehaskell{<thunk: expression-to-be-evaluated>}\bigskip
\begin{itemize}
...
\haskellcode{src/profiling.hs}\bigskip
\end{frame}
...
\inlinehaskell{seq} evaluates its first argument to \textbf{WHNF}, and return the second one.\bigskip
...
\inlinehaskell{deepseq :: NFData a => a -> b -> b}\newline\newline
\inlinehaskell{($!!) :: NFData a => (a -> b) -> a -> b  -- |infixr 0|}\newline\newline
\inlinehaskell{force :: NFData a => a -> a}\newline
\inlinehaskell{force x = x `deepseq` x}\newline
\haskellcode{./src/NFData.hs}
